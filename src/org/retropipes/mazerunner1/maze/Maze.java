...
public class Maze implements Serializable {
    ...
    public Maze(final int rows, final int cols, final int floors, final int levels) {
    ...
    // Accessors
    public MazeObject getCell(final int row, final int col, final int floor, final int level) {
	...
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
		    if (this.mazeData[x][y][z][level].getName().equals("Player")) {
