...
import org.retropipes.mazerunner1.objects.base.MazeObjectList;
...
public class MazeMaker {
    ...
    private static int editorLocationZ, editorLocationW, editorLocationX, editorLocationY, editorCameFromZ,
	    editorCameFromW;
	...
	MazeMaker.app.setViewingWindowLocation(false, MazeMaker.app.getViewingWindowLocation(false) + x);
	MazeMaker.app.setViewingWindowLocation(true, MazeMaker.app.getViewingWindowLocation(true) + y);
	MazeMaker.app.setLowerViewingWindowLocation(false, MazeMaker.app.getLowerViewingWindowLocation(false) + x);
	MazeMaker.app.setLowerViewingWindowLocation(true, MazeMaker.app.getLowerViewingWindowLocation(true) + y);
	...
	if (MazeMaker.app.getViewingWindowLocation(false) < MazeRunner.MIN_VIEWING_WINDOW_X) {
	    MazeMaker.app.setViewingWindowLocation(false, MazeRunner.MIN_VIEWING_WINDOW_X);
	    MazeMaker.app.setLowerViewingWindowLocation(false,
		    MazeRunner.MIN_VIEWING_WINDOW_X + MazeRunner.VIEWING_WINDOW_SIZE_X - 1);
	}
	if (MazeMaker.app.getViewingWindowLocation(true) < MazeRunner.MIN_VIEWING_WINDOW_Y) {
	    MazeMaker.app.setViewingWindowLocation(true, MazeRunner.MIN_VIEWING_WINDOW_Y);
	    MazeMaker.app.setLowerViewingWindowLocation(true,
		    MazeRunner.MIN_VIEWING_WINDOW_Y + MazeRunner.VIEWING_WINDOW_SIZE_Y - 1);
	}
	if (MazeMaker.app.getLowerViewingWindowLocation(false) > MazeMaker.MAX_VIEWING_WINDOW_X) {
	    MazeMaker.app.setLowerViewingWindowLocation(false, MazeMaker.MAX_VIEWING_WINDOW_X);
	    MazeMaker.app.setViewingWindowLocation(false,
		    MazeMaker.MAX_VIEWING_WINDOW_X - MazeRunner.VIEWING_WINDOW_SIZE_X + 1);
	}
	if (MazeMaker.app.getLowerViewingWindowLocation(true) > MazeMaker.MAX_VIEWING_WINDOW_Y) {
	    MazeMaker.app.setLowerViewingWindowLocation(true, MazeMaker.MAX_VIEWING_WINDOW_Y);
	    MazeMaker.app.setViewingWindowLocation(true,
		    MazeMaker.MAX_VIEWING_WINDOW_Y - MazeRunner.VIEWING_WINDOW_SIZE_Y + 1);
	}
	...
	for (x = MazeMaker.app.getViewingWindowLocation(false); x <= MazeMaker.app
		...
		try {
		    MazeMaker.secondaryPane.add(new JLabel("", MazeMaker.app.getMaze()
			    .getCell(y, x, MazeMaker.editorLocationZ, MazeMaker.editorLocationW).getEditorAppearance(),
			    SwingConstants.CENTER));
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    MazeMaker.secondaryPane.add(
			    new JLabel("", ImageSetManager.getImage(MazeObject.getSet(), MazeObject.getSize(), "Void"),
				    SwingConstants.CENTER));
		}
	    }
	}
	MazeMaker.borderPane.add(MazeMaker.outputPane, BorderLayout.CENTER);
	MazeMaker.borderPane.add(MazeMaker.app.getMessageLabel(), BorderLayout.SOUTH);
	MazeMaker.outputFrame.pack();
	MazeMaker.outputFrame.setTitle(
		"Maze Maker - Floor " + (MazeMaker.editorLocationZ + 1) + " Level " + (MazeMaker.editorLocationW + 1));
	...
	int z = 0;
	MazeObject mo = null;
	final int xOffset = MazeMaker.vertScroll.getValue() - MazeMaker.vertScroll.getMinimum();
	final int yOffset = MazeMaker.horzScroll.getValue() - MazeMaker.horzScroll.getMinimum();
	final int gridX = x / MazeObject.getSize() + MazeMaker.app.getViewingWindowLocation(false) - xOffset + yOffset;
	final int gridY = y / MazeObject.getSize() + MazeMaker.app.getViewingWindowLocation(true) + xOffset - yOffset;
	try {
	    MazeMaker.app.setSavedMazeObject(MazeMaker.app.getMaze().getCell(gridX, gridY, MazeMaker.editorLocationZ,
	...
	if (result != null && result.length() > 0) {
	    ...
	    try {
		MazeMaker.checkTwoWayTeleporterPair(MazeMaker.editorLocationZ, MazeMaker.editorLocationW);
		MazeMaker.updateUndoHistory(MazeMaker.app.getSavedMazeObject(), gridX, gridY, MazeMaker.editorLocationZ,
			MazeMaker.editorLocationW);
		MazeMaker.app.getMaze().setCell(mo, gridX, gridY, MazeMaker.editorLocationZ, MazeMaker.editorLocationW);
		MazeMaker.checkStairPair(MazeMaker.editorLocationZ, MazeMaker.editorLocationW);
		MazeMaker.app.setDirty(true);
		MazeMaker.redrawEditor();
	    } catch (final NullPointerException np) {
		MazeMaker.app.getMaze().setCell(MazeMaker.app.getSavedMazeObject(), gridX, gridY,
	...
	final MazeObject mo1 = MazeMaker.app.getMaze().getCell(MazeMaker.editorLocationX, MazeMaker.editorLocationY, z,
		w);
	final String name1 = mo1.getName();
	String name2, name3;
	try {
	    final MazeObject mo2 = MazeMaker.app.getMaze().getCell(MazeMaker.editorLocationX, MazeMaker.editorLocationY,
		    z + 1, w);
	    name2 = mo2.getName();
	...
	try {
	    final MazeObject mo3 = MazeMaker.app.getMaze().getCell(MazeMaker.editorLocationX, MazeMaker.editorLocationY,
		    z - 1, w);
	    name3 = mo3.getName();
	} catch (final ArrayIndexOutOfBoundsException e) {
	    name3 = "";
	...
	if (name1.equals("Two-Way Teleporter")) {
	    ...
	    final MazeObject mo2 = MazeMaker.app.getMaze().getCell(destX, destY, destZ, destW);
	    name2 = mo2.getName();
    ...
    private static void reverseCheckTwoWayTeleporterPair(final int z, final int w) {
	final MazeObject mo1 = MazeMaker.app.getMaze().getCell(MazeMaker.editorLocationX, MazeMaker.editorLocationY, z,
		w);
	final String name1 = mo1.getName();
	String name2;
	...
	case TELEPORTER_TYPE_RANDOM_INVISIBLE_ONESHOT:
	    input1 = JOptionPane.showInputDialog(MazeMaker.outputFrame, "Random row range:", "Maze Maker",
	...
	if (input1 != null) {
	    ...
	    case TELEPORTER_TYPE_RANDOM_ONESHOT:
	    case TELEPORTER_TYPE_RANDOM_INVISIBLE_ONESHOT:
		input2 = JOptionPane.showInputDialog(MazeMaker.outputFrame, "Random column range:", "Maze Maker",
			JOptionPane.OK_CANCEL_OPTION);
		break;
	    default:
		break;
	    ...
	    case TELEPORTER_TYPE_INVISIBLE_ONESHOT:
	    case TELEPORTER_TYPE_TWOWAY:
		MazeMaker.horzScroll.removeAdjustmentListener(MazeMaker.mhandler);
		MazeMaker.vertScroll.removeAdjustmentListener(MazeMaker.mhandler);
	...
	String input1 = null;
	int destW = 0;
	input1 = JOptionPane.showInputDialog(MazeMaker.outputFrame, "Destination Level:", "Maze Maker",
		JOptionPane.OK_CANCEL_OPTION);
	if (input1 != null) {
	    try {
	...
	final int xOffset = MazeMaker.vertScroll.getValue() - MazeMaker.vertScroll.getMinimum();
	final int yOffset = MazeMaker.horzScroll.getValue() - MazeMaker.horzScroll.getMinimum();
	final int destX = x / MazeObject.getSize() + MazeMaker.app.getViewingWindowLocation(false) - xOffset + yOffset;
	final int destY = y / MazeObject.getSize() + MazeMaker.app.getViewingWindowLocation(true) + xOffset - yOffset;
	...
	MazeMaker.app.setPlayerLocation(false, false, MazeMaker.editorLocationX);
    }
	...
	try {
		...
		for (w = 0; w < MazeMaker.app.getMaze().getLevels(); w++) {
			...
			for (y = 0; y < MazeMaker.app.getMaze().getRows(); y++) {
			    for (x = 0; x < MazeMaker.app.getMaze().getColumns(); x++) {
				mazeFile.print(MazeMaker.app.getMaze().getCell(x, y, z, w).toString());
				mazeFile.print("\n");
			    }
			}
		    }
		...
		throw new InvalidMazeException("Maze file couldn't be written to.");
	    }
	} catch (final InvalidMazeException ime) {
	    if (MazeMaker.app.getMessageEnabled(MazeRunner.MESSAGE_SPECIFIC_MAZE_ERRORS)) {
	...
	if (MazeMaker.app.getLoaded()) {
		...
		MazeMaker.outputFrame.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
		MazeMaker.borderPane.add(MazeMaker.outputPane, BorderLayout.CENTER);
		MazeMaker.borderPane.add(MazeMaker.app.getMessageLabel(), BorderLayout.SOUTH);
		...
		MazeMaker.secondaryPane
			.setLayout(new GridLayout(MazeRunner.VIEWING_WINDOW_SIZE_X, MazeRunner.VIEWING_WINDOW_SIZE_Y));
		MazeMaker.horzScroll = new JScrollBar(Adjustable.HORIZONTAL, MazeRunner.MIN_VIEWING_WINDOW_Y,
			MazeRunner.VIEWING_WINDOW_SIZE_Y, MazeRunner.MIN_VIEWING_WINDOW_Y,
			MazeMaker.MAX_VIEWING_WINDOW_Y);
		MazeMaker.vertScroll = new JScrollBar(Adjustable.VERTICAL, MazeRunner.MIN_VIEWING_WINDOW_X,
			MazeRunner.VIEWING_WINDOW_SIZE_X, MazeRunner.MIN_VIEWING_WINDOW_X,
			MazeMaker.MAX_VIEWING_WINDOW_X);
		MazeMaker.c.gridx = 0;
		MazeMaker.c.gridy = 0;
		MazeMaker.gridbag.setConstraints(MazeMaker.secondaryPane, MazeMaker.c);
		MazeMaker.outputPane.add(MazeMaker.secondaryPane);
		MazeMaker.c.gridx = 1;
		MazeMaker.c.gridy = 0;
		MazeMaker.c.gridwidth = GridBagConstraints.REMAINDER;
		MazeMaker.gridbag.setConstraints(MazeMaker.vertScroll, MazeMaker.c);
		MazeMaker.outputPane.add(MazeMaker.vertScroll);
		MazeMaker.c.gridx = 0;
		MazeMaker.c.gridy = 1;
		MazeMaker.c.gridwidth = 1;
		MazeMaker.c.gridheight = GridBagConstraints.REMAINDER;
		MazeMaker.gridbag.setConstraints(MazeMaker.horzScroll, MazeMaker.c);
		MazeMaker.outputPane.add(MazeMaker.horzScroll);
		MazeMaker.horzScroll.addAdjustmentListener(MazeMaker.mhandler);
		MazeMaker.vertScroll.addAdjustmentListener(MazeMaker.mhandler);
		MazeMaker.secondaryPane.addMouseListener(MazeMaker.mhandler);
		MazeMaker.outputFrame.addKeyListener(MazeMaker.mhandler);
		MazeMaker.outputFrame.addWindowListener(MazeMaker.app.getEventHandler());
	...
	if (saved) {
	    input1 = JOptionPane.showInputDialog(null, "Number of rows?", "New Maze", JOptionPane.OK_CANCEL_OPTION);
	    if (input1 != null) {
		input2 = JOptionPane.showInputDialog(null, "Number of columns?", "New Maze",
			JOptionPane.OK_CANCEL_OPTION);
		if (input2 != null) {
		    input3 = JOptionPane.showInputDialog(null, "Number of floors?", "New Maze",
			    JOptionPane.OK_CANCEL_OPTION);
		    if (input3 != null) {
			input4 = JOptionPane.showInputDialog(null, "Number of levels?", "New Maze",
				JOptionPane.OK_CANCEL_OPTION);
			if (input4 != null) {
			    try {
				...
				if (mazeSizeX < 1 || mazeSizeY < 1 || mazeSizeZ < 1 || mazeSizeW < 1) {
				    throw new NumberFormatException();
				}
				MazeMaker.app.setPlayerLocation(false, false, -1);
				MazeMaker.app.setPlayerLocation(false, true, -1);
				MazeMaker.app.setPlayerLocation(true, false, -1);
				MazeMaker.app.setPlayerLocation(true, true, -1);
				MazeMaker.app.setMaze(new Maze(mazeSizeX, mazeSizeY, mazeSizeZ, mazeSizeW));
				MazeMaker.app.setViewingWindowLocation(false,
				...
				MazeMaker.app.setLowerViewingWindowLocation(true,
					0 + (MazeRunner.VIEWING_WINDOW_SIZE_Y - 1) / 2);
				MazeMaker.MAX_VIEWING_WINDOW_X = mazeSizeX + MazeRunner.VIEWING_WINDOW_SIZE_X / 2 - 1;
				MazeMaker.MAX_VIEWING_WINDOW_Y = mazeSizeY + MazeRunner.VIEWING_WINDOW_SIZE_Y / 2 - 1;
    ...
    private static void updateUndoHistory(final MazeObject obj, final int x, final int y, final int z, final int w) {
	MazeMaker.engine.updateUndoHistory(obj, x, y, z, w);
    }

    private static void updateRedoHistory(final MazeObject obj, final int x, final int y, final int z, final int w) {
