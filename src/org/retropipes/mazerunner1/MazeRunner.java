...
public class MazeRunner {
    ...
    private Container outputPane, mainPrefPane, subPrefPane1, subPrefPane2, menuPane, borderPane;
    private JButton prefsOK, prefsCancel;
    private JComboBox<String> sets, sizes;
    private JCheckBox msgCannotGoOutsideMaze, msgFakeExit, msgInvisibleWall, msgCannotGoThatWay, msgSpecificMazeErrors,
	    msgInvisibleTeleporter;
    ...
    private JMenuItem editUndo, editRedo, editCut, editCopy, editPaste, editPreferences, editClearHistory;
    ...
    private KeyStroke editUndoAccel, editRedoAccel, editCutAccel, editCopyAccel, editPasteAccel, editPreferencesAccel,
    ...
    private int playerLocationX, playerLocationY, playerLocationZ, playerLocationW;
    private int mazeSizeX, mazeSizeY, mazeSizeZ, mazeSizeW;
    private int oldPlayerLocationX, oldPlayerLocationY, oldPlayerLocationZ, oldPlayerLocationW;
    private int upperLeftViewingWindowX, upperLeftViewingWindowY, lowerRightViewingWindowX, lowerRightViewingWindowY;
    private int oldUpperLeftViewingWindowX, oldUpperLeftViewingWindowY, oldLowerRightViewingWindowX,
	    oldLowerRightViewingWindowY;
    ...
    public MazeObject getMazeObject(final int x, final int y, final int z, final int w) {
	try {
	...
	try {
	    this.gameMaze.getCell(this.playerLocationX, this.playerLocationY, this.playerLocationZ - 1,
	...
	try {
	    this.gameMaze.getCell(this.playerLocationX, this.playerLocationY, this.playerLocationZ,
	...
	try {
	    this.gameMaze.getCell(this.playerLocationX, this.playerLocationY, this.playerLocationZ, level);
	    return true;
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    return false;
	}
	...
	int status = 0;
	if (this.IN_GAME) {
	    status = JOptionPane.showConfirmDialog(this.outputFrame, "Do you want to save your game?", "Maze Runner",
		    JOptionPane.YES_NO_CANCEL_OPTION);
	} else {
	    status = JOptionPane.showConfirmDialog(MazeMaker.getOutputFrame(), "Do you want to save your maze?",
		    "Maze Maker", JOptionPane.YES_NO_CANCEL_OPTION);
	}
	return status;
    }
	...
	do {
	    try {
		o = this.gameMaze.getCell(this.playerLocationX + x, this.playerLocationY + y, this.playerLocationZ,
			this.playerLocationW);
		o.playSound();
	    ...
	    try {
		if (!this.savedMazeObject.isConditionallyDirectionallySolid(false, this.playerLocationX + x,
			this.playerLocationY + y, this.inv)) {
		    if (!this.gameMaze.getCell(this.playerLocationX + x, this.playerLocationY + y, this.playerLocationZ,
			    this.playerLocationW).isConditionallyDirectionallySolid(true, this.playerLocationX + x,
				    this.playerLocationY + y, this.inv)) {
			...
			if (acted.isPullable() && this.isPullInProgress()) {
			    ...
			    if (savedObject.doesAcceptPullOut() && pulledInto.doesAcceptPullInto()) {
				savedObject.pullOutAction(this.inv);
				acted.pullAction(this.inv, pulledInto, x, y, pullX, pullY);
			...
			this.redrawMaze();
			this.isDirty = true;
			this.savedMazeObject.postMoveAction(false, this.playerLocationX, this.playerLocationY,
				this.inv);
		    } else {
			acted = this.gameMaze.getCell(this.playerLocationX + x, this.playerLocationY + y,
				this.playerLocationZ, this.playerLocationW);
			if (acted.isPushable()) {
			    ...
			    pushedInto = this.gameMaze.getCell(this.playerLocationX + pushX,
				    this.playerLocationY + pushY, this.playerLocationZ, this.playerLocationW);
			    if (savedObject.doesAcceptPushOut() && pushedInto.doesAcceptPushInto()) {
				this.gameMaze.setCell(this.savedMazeObject, this.playerLocationX, this.playerLocationY,
					this.playerLocationZ, this.playerLocationW);
				savedObject.pushOutAction(this.inv);
				acted.pushAction(this.inv, pushedInto, x, y, pushX, pushY);
				pushedInto.pushIntoAction(this.inv, acted, this.playerLocationX + pushX,
					this.playerLocationY + pushY, this.playerLocationZ, this.playerLocationW);
			    ...
			    acted.chainReactionAction(this.playerLocationX + x, this.playerLocationY + y,
				    this.playerLocationZ, this.playerLocationW);
			}
		    }
		}
    ...
    public void updatePushedPosition(final int x, final int y, final int pushX, final int pushY, final MazeObject o1,
	    final MazeObject o2) {
	try {
	    this.gameMaze.setCell(o1, this.playerLocationX + x, this.playerLocationY + y, this.playerLocationZ,
		    this.playerLocationW);
	    this.gameMaze.setCell(o2, this.playerLocationX + pushX, this.playerLocationY + pushY, this.playerLocationZ,
		    this.playerLocationW);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Do nothing
	}
    ...
    public void updatePulledPosition(final int x, final int y, final int pullX, final int pullY, final MazeObject o1,
	    final MazeObject o2) {
	try {
	    this.gameMaze.setCell(o1, this.playerLocationX - x, this.playerLocationY - y, this.playerLocationZ,
		    this.playerLocationW);
	    this.gameMaze.setCell(o2, this.playerLocationX - pullX, this.playerLocationY - pullY, this.playerLocationZ,
		    this.playerLocationW);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Do nothing
	}
	...
	try {
		...
		pushedInto.getSavedObject().pushIntoAction(this.inv, pushedInto, x2, y2, z2 - 1, w2);
		this.redrawMaze();
		this.isDirty = true;
	    }
	} catch (final ArrayIndexOutOfBoundsException ae) {
	...
	try {
	    ...
	    this.gameMaze.getCell(x, y, z, w).preMoveAction(true, x, y, this.inv);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Do nothing
	}
	this.oldPlayerLocationX = this.playerLocationX;
	...
	try {
		...
		this.upperLeftViewingWindowX = this.playerLocationY - (MazeRunner.VIEWING_WINDOW_SIZE_X - 1) / 2;
		this.upperLeftViewingWindowY = this.playerLocationX - (MazeRunner.VIEWING_WINDOW_SIZE_Y - 1) / 2;
		this.lowerRightViewingWindowX = this.playerLocationY + (MazeRunner.VIEWING_WINDOW_SIZE_X - 1) / 2;
	...
	try {
	    this.gameMaze
		    .getCell(this.playerLocationX, this.playerLocationY, this.playerLocationZ, this.playerLocationW)
		    .playSound();
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Do nothing
	}
	...
	if (this.outputFrame.isVisible()) {
		...
		for (y = this.upperLeftViewingWindowY; y <= this.lowerRightViewingWindowY; y++) {
			...
			this.outputPane.add(new JLabel(
				"", this.gameMaze.getCell(y, x, this.playerLocationZ, this.playerLocationW)
					.gameRenderHook(y, x, this.playerLocationZ, this.playerLocationW),
	...
	final boolean playerExists = this.gameMaze.findPlayerOnLevel(this.playerLocationW + 1);
	if (playerExists) {
	    this.gameMaze.restoreLevel(this.playerLocationW);
	    this.playerLocationX = this.gameMaze.getStartColumn(this.playerLocationW + 1);
	    this.playerLocationY = this.gameMaze.getStartRow(this.playerLocationW + 1);
	    this.playerLocationZ = this.gameMaze.getStartFloor(this.playerLocationW + 1);
	    this.playerLocationW = this.gameMaze.getStartLevel();
	    this.upperLeftViewingWindowX = this.playerLocationX - (MazeRunner.VIEWING_WINDOW_SIZE_X - 1) / 2;
	    this.upperLeftViewingWindowY = this.playerLocationY - (MazeRunner.VIEWING_WINDOW_SIZE_Y - 1) / 2;
	    this.lowerRightViewingWindowX = this.playerLocationX + (MazeRunner.VIEWING_WINDOW_SIZE_X - 1) / 2;
	    this.lowerRightViewingWindowY = this.playerLocationY + (MazeRunner.VIEWING_WINDOW_SIZE_Y - 1) / 2;
	    this.decay();
	    this.redrawMaze();
	} else {
	    this.solvedMaze();
	...
	if (this.IN_GAME) {
	    ...
	    if (playerExists) {
		this.playerLocationW = this.gameMaze.getStartLevel();
		this.playerLocationX = this.gameMaze.getStartColumn(this.playerLocationW);
		this.playerLocationY = this.gameMaze.getStartRow(this.playerLocationW);
		this.playerLocationZ = this.gameMaze.getStartFloor(this.playerLocationW);
		this.upperLeftViewingWindowX = this.playerLocationX - (MazeRunner.VIEWING_WINDOW_SIZE_X - 1) / 2;
		this.upperLeftViewingWindowY = this.playerLocationY - (MazeRunner.VIEWING_WINDOW_SIZE_Y - 1) / 2;
		this.lowerRightViewingWindowX = this.playerLocationX + (MazeRunner.VIEWING_WINDOW_SIZE_X - 1) / 2;
		this.lowerRightViewingWindowY = this.playerLocationY + (MazeRunner.VIEWING_WINDOW_SIZE_Y - 1) / 2;
    ...
    public void setPlayerLocation(final boolean outer, final boolean inner, final int newLoc) {
	if (outer) {
	    if (inner) {
    ...
    public void setLowerViewingWindowLocation(final boolean flag, final int newCoord) {
	if (flag) {
	...
	if (this.usingAnItem() && this.IN_GAME) {
	    final int xOffset = this.upperLeftViewingWindowX - MazeRunner.MIN_VIEWING_WINDOW_X;
	    final int yOffset = this.upperLeftViewingWindowY - MazeRunner.MIN_VIEWING_WINDOW_Y;
	    final int destX = x / MazeObject.getSize() + this.upperLeftViewingWindowX - xOffset + yOffset;
	    final int destY = y / MazeObject.getSize() + this.upperLeftViewingWindowY + xOffset - yOffset;
	...
	try {
	    try (BufferedReader mazeFile = new BufferedReader(new FileReader(filename))) {
		try {
		    ...
		    this.playerLocationW = -1;
		    this.gameMaze = new Maze(this.mazeSizeX, this.mazeSizeY, this.mazeSizeZ, this.mazeSizeW);
		    ...
		    if (this.playerLocationX == -1 || this.playerLocationY == -1 || this.playerLocationZ == -1
			    || this.playerLocationW == -1) {
			throw new InvalidMazeException("No player found.");
		    } else {
			final boolean playerExists = this.gameMaze.findPlayerOnLevel(0);
			if (playerExists) {
			    this.playerLocationX = this.gameMaze.getStartColumn(0);
			    this.playerLocationY = this.gameMaze.getStartRow(0);
			    this.playerLocationZ = this.gameMaze.getStartFloor(0);
			    this.playerLocationW = this.gameMaze.getStartLevel();
			...
			throw new InvalidMazeException("Maze file could not be closed.");
		    }
		    if (w != this.mazeSizeW) {
			throw new InvalidMazeException("Maze file too small.");
		    }
		    MazeMaker.setMaxViewingWindow(this.mazeSizeX + MazeRunner.VIEWING_WINDOW_SIZE_X / 2,
			    this.mazeSizeY + MazeRunner.VIEWING_WINDOW_SIZE_Y / 2);
		} catch (final IOException ie) {
	...
	try {
		...
		try {
			...
			if (playerExists) {
			    this.playerLocationX = this.gameMaze.getStartColumn(startW);
			    this.playerLocationY = this.gameMaze.getStartRow(startW);
			    this.playerLocationZ = this.gameMaze.getStartFloor(startW);
			...
			if (!temp.equals("INVENTORY")) {
			    throw new InvalidMazeException("No inventory state found.");
			} else {
			...
			throw new InvalidMazeException("Saved game file could not be closed.");
		    }
		    if (w != this.mazeSizeW) {
			throw new InvalidMazeException("Saved game file too small.");
		    }
		    MazeMaker.setMaxViewingWindow(this.mazeSizeX + MazeRunner.VIEWING_WINDOW_SIZE_X / 2,
			    this.mazeSizeY + MazeRunner.VIEWING_WINDOW_SIZE_Y / 2);
		} catch (final IOException ie) {
		    throw new InvalidMazeException("Error reading saved game file.");
		}
	    } catch (final IOException ie) {
		throw new InvalidMazeException("Saved game file couldn't be opened.");
	    }
	} catch (final InvalidMazeException ime) {
	    if (this.getMessageEnabled(MazeRunner.MESSAGE_SPECIFIC_MAZE_ERRORS)) {
		Messager.showDialog(ime.getMessage());
    ...
    private void textMazeParser(final int x, final int y, final int z, final int w, final BufferedReader mazeFile)
	...
	boolean success = true;
	try {
	    try (PrintWriter mazeFile = new PrintWriter(new BufferedWriter(new FileWriter(filename)))) {
		...
		for (w = 0; w < this.mazeSizeW; w++) {
			...
			for (y = 0; y < this.mazeSizeY; y++) {
			    for (x = 0; x < this.mazeSizeX; x++) {
				mazeFile.print(this.gameMaze.getCell(x, y, z, w).toString());
				mazeFile.print("\n");
			    }
			}
		    }
		...
		throw new InvalidMazeException("Saved game file couldn't be written to.");
	    }
	} catch (final InvalidMazeException ime) {
	    if (this.getMessageEnabled(MazeRunner.MESSAGE_SPECIFIC_MAZE_ERRORS)) {
		Messager.showDialog(ime.getMessage());
	...
	if (this.loaded) {
		...
		this.borderPane.add(this.messageLabel, BorderLayout.SOUTH);
		this.outputPane
			.setLayout(new GridLayout(MazeRunner.VIEWING_WINDOW_SIZE_X, MazeRunner.VIEWING_WINDOW_SIZE_Y));
	...
	if (result) {
	    ...
	    this.setMessageEnabled(MazeRunner.MESSAGE_CANNOT_GO_OUTSIDE_MAZE, false);
	}
	...
	if (result) {
	    this.setMessageEnabled(MazeRunner.MESSAGE_SPECIFIC_MAZE_ERRORS, true);
	} else {
	...
	if (result) {
	    ...
	    this.setMessageEnabled(MazeRunner.MESSAGE_INVISIBLE_TELEPORTER, false);
	}
	...
	this.msgCannotGoOutsideMaze = new JCheckBox("Enable \"Cannot go outside maze\" message", true);
	this.msgFakeExit = new JCheckBox("Enable \"Fake exit!\" message", true);
	this.msgInvisibleWall = new JCheckBox("Enable \"Invisible wall!\" message", true);
	this.msgCannotGoThatWay = new JCheckBox("Enable \"Cannot go that way\" message", true);
	this.msgSpecificMazeErrors = new JCheckBox("Enable specific maze error messages", false);
	this.msgInvisibleTeleporter = new JCheckBox("Enable \"Invisible Teleporter!\" message", true);
	...
	if (System.getProperty("os.name").equalsIgnoreCase("Mac OS X")) {
	    this.fileNewAccel = KeyStroke.getKeyStroke(KeyEvent.VK_N, InputEvent.META_DOWN_MASK);
	    this.fileOpenAccel = KeyStroke.getKeyStroke(KeyEvent.VK_O, InputEvent.META_DOWN_MASK);
	    this.fileCloseAccel = KeyStroke.getKeyStroke(KeyEvent.VK_W, InputEvent.META_DOWN_MASK);
	    this.fileSaveAccel = KeyStroke.getKeyStroke(KeyEvent.VK_S, InputEvent.META_DOWN_MASK);
	    this.fileSaveAsAccel = KeyStroke.getKeyStroke(KeyEvent.VK_S,
		    InputEvent.META_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK);
	    this.editUndoAccel = KeyStroke.getKeyStroke(KeyEvent.VK_Z, InputEvent.META_DOWN_MASK);
	    this.editRedoAccel = KeyStroke.getKeyStroke(KeyEvent.VK_Z,
		    InputEvent.META_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK);
	    this.editCutAccel = KeyStroke.getKeyStroke(KeyEvent.VK_X, InputEvent.META_DOWN_MASK);
	    this.editCopyAccel = KeyStroke.getKeyStroke(KeyEvent.VK_C, InputEvent.META_DOWN_MASK);
	    this.editPasteAccel = KeyStroke.getKeyStroke(KeyEvent.VK_V, InputEvent.META_DOWN_MASK);
	    this.editPreferencesAccel = KeyStroke.getKeyStroke(KeyEvent.VK_PERIOD, InputEvent.META_DOWN_MASK);
	    this.editClearHistoryAccel = KeyStroke.getKeyStroke(KeyEvent.VK_Y, InputEvent.META_DOWN_MASK);
	    this.playPlayMazeAccel = KeyStroke.getKeyStroke(KeyEvent.VK_P, InputEvent.META_DOWN_MASK);
	    this.playEditMazeAccel = KeyStroke.getKeyStroke(KeyEvent.VK_E, InputEvent.META_DOWN_MASK);
	    this.gameInventoryAccel = KeyStroke.getKeyStroke(KeyEvent.VK_I, InputEvent.META_DOWN_MASK);
	    this.gameUseAccel = KeyStroke.getKeyStroke(KeyEvent.VK_U, InputEvent.META_DOWN_MASK);
	} else {
	    this.fileNewAccel = KeyStroke.getKeyStroke(KeyEvent.VK_N, InputEvent.CTRL_DOWN_MASK);
	    this.fileOpenAccel = KeyStroke.getKeyStroke(KeyEvent.VK_O, InputEvent.CTRL_DOWN_MASK);
	    this.fileCloseAccel = KeyStroke.getKeyStroke(KeyEvent.VK_W, InputEvent.CTRL_DOWN_MASK);
	    this.fileSaveAccel = KeyStroke.getKeyStroke(KeyEvent.VK_S, InputEvent.CTRL_DOWN_MASK);
	    this.fileSaveAsAccel = KeyStroke.getKeyStroke(KeyEvent.VK_S,
		    InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK);
	    this.editUndoAccel = KeyStroke.getKeyStroke(KeyEvent.VK_Z, InputEvent.CTRL_DOWN_MASK);
	    this.editRedoAccel = KeyStroke.getKeyStroke(KeyEvent.VK_Z,
		    InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK);
	    this.editCutAccel = KeyStroke.getKeyStroke(KeyEvent.VK_X, InputEvent.CTRL_DOWN_MASK);
	    this.editCopyAccel = KeyStroke.getKeyStroke(KeyEvent.VK_C, InputEvent.CTRL_DOWN_MASK);
	    this.editPasteAccel = KeyStroke.getKeyStroke(KeyEvent.VK_V, InputEvent.CTRL_DOWN_MASK);
	    this.editPreferencesAccel = KeyStroke.getKeyStroke(KeyEvent.VK_COMMA, InputEvent.CTRL_DOWN_MASK);
	    this.editClearHistoryAccel = KeyStroke.getKeyStroke(KeyEvent.VK_Y, InputEvent.CTRL_DOWN_MASK);
	    this.playPlayMazeAccel = KeyStroke.getKeyStroke(KeyEvent.VK_P, InputEvent.CTRL_DOWN_MASK);
	    this.playEditMazeAccel = KeyStroke.getKeyStroke(KeyEvent.VK_E, InputEvent.CTRL_DOWN_MASK);
	    this.gameInventoryAccel = KeyStroke.getKeyStroke(KeyEvent.VK_I, InputEvent.CTRL_DOWN_MASK);
	    this.gameUseAccel = KeyStroke.getKeyStroke(KeyEvent.VK_U, InputEvent.CTRL_DOWN_MASK);
	}
    }
