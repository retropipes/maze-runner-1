...
public class SoundManager {
	...
	try {
	    ...
	    SoundManager.stream = AudioSystem.getAudioInputStream(SoundManager.file);
	    SoundManager.fileFormat = AudioSystem.getAudioFileFormat(SoundManager.file);
	    SoundManager.format = SoundManager.fileFormat.getFormat();
	    SoundManager.info = new DataLine.Info(Clip.class, SoundManager.format);
	    if (!AudioSystem.isLineSupported(SoundManager.info)) {
		// Do nothing
	    }
	    try {
		SoundManager.clip = (Clip) AudioSystem.getLine(SoundManager.info);
